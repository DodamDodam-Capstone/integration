...
```text
Jira DOD-100: 사용자 인증 Epic
└─ integration#20 [DOD-100] 사용자 인증
   ├─ frontend#12 [DOD-101] 로그인 화면
   ├─ backend#18  [DOD-102] 인증 API
   ├─ ai#7        [DOD-103] 이상 로그인 판별
   └─ integration#21 [DOD-104] 인증 서비스 통합 검증
```
...
```text
feature/DOD-101-login-page
fix/DOD-102-refresh-token
hotfix/DOD-103-auth-failure
```

### Commit

```text
✨ feat(auth): DOD-101 로그인 화면 추가
🐛 fix(auth): DOD-102 만료된 refresh token 거부
```
...
```text
✨ feat(auth): [DOD-101] 로그인 화면 추가
🐛 fix(auth): [DOD-102] refresh token 검증 수정
```
...
```text
🚀 chore(release): [DOD-100] development를 main으로 승격
```
...
```text
⬆️ deps(frontend): [DOD-100] 컴포넌트를 8e973e2로 갱신
```
...
Jira Smart Commit은 `DOD-101 #done` 같은 상태 변경 명령을 지원합니다. 하지만
