...
통과하고 모든 review conversation을 해결합니다. 작업 브랜치와 Integration Bot
PR → `development`는 squash merge하고, `development` → `main` 승격은 merge
commit을 사용합니다. 자동 브랜치 삭제는 사용하지 않으며 작업 브랜치는 sprint
정리 시 수동으로 삭제합니다.
